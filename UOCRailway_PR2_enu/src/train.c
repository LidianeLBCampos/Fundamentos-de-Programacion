...
/* The content of the fields of the loaded train structure is written into the string str */
void getLoadedTrainStr(tLoadedTrain loadedTrain, int maxSize, char *str) {
	...
	length = snprintf(str,maxSize-1,"%d %d %d", 
             (int)loadedTrain.id, (int)loadedTrain.load.id, loadedTrain.load.qtt);
	if (length>0) 
		str[length]='\0';
}

/* The content of the string str is parsed into the fields of the loaded train structure */
void getLoadedTrainObject(const char *str, tLoadedTrain *loadedTrain) {

	int aux_train_id, aux_good_id;
 		
    /* read train data */
    sscanf(str, "%d %d %d", 
             &aux_train_id, &aux_good_id, &(loadedTrain->load.qtt));

    loadedTrain->id = (tTrainId)aux_train_id;
    loadedTrain->load.id = (tGoodId)aux_good_id;
}
...
tTrain trainsTable_get(tTrainTable tabTrains, tTrainId id)
{
	...
	int i=0;
	while(i< tabTrains.nTrains && train.id==NO_TRAIN) {
		/* Check if the id is the same */
		if(id==tabTrains.table[i].id)  {
			train_cpy(&train,tabTrains.table[i]);
		}
		i++;
	}
